...
gamebian_have_loginusers_vdf_for_home() {
	...
	for _gf in "${_home}/.local/share/Steam/config/loginusers.vdf" \
		"${_home}/.steam/steam/config/loginusers.vdf" \
...
gamebian_steam_process_busy() {
	...
	if [ -f "${HOME}/.steam/steam/.needs-steam-bootstrap" ] \
		|| [ -f "${HOME}/.steam/debian-installation/.needs-steam-bootstrap" ] \
...
gamebian_steam_client_installed() {
	...
	if [ -f "${HOME}/.steam/steam/ubuntu12_32/steam" ] \
		|| [ -f "${HOME}/.steam/debian-installation/ubuntu12_32/steam" ] \
		|| [ -f "${HOME}/.local/share/Steam/ubuntu12_32/steam" ] \
		|| [ -x "${HOME}/.steam/steam/steam.sh" ] \
