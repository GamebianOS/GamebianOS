...
- **Steam (installed disk):** first login opens the **Steam setup** terminal (**`/usr/share/gamebian/steam-firstboot-terminal.sh`**). After **`~/.config/gamebian-firstboot-steam.done`**, subsequent logins autostart **nested gamescope** + **Steam gamepad UI** (**`/usr/local/bin/gamebian-steam-bigpicture`**, **`gamescope -e -f -- steam -gamepadui`** — **`-f`** fullscreen under X11; set **`GAMEBIAN_GAMESCOPE_WINDOWED=1`** to use a small window). Quitting Steam returns to Openbox; open again from the **Steam Big Picture** menu entry or **Super+Shift+S**. Set **`STEAM_BIGPICTURE_UI=classic`** for **`-bigpicture`**. Keep Steam’s own **run at login** disabled to avoid a second client.
