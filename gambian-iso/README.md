...
- **Steam (installed disk):** **Phase 1 —** until **`~/.config/gamebian-firstboot-steam.done`** exists, first Openbox login opens **xfce4-terminal** running **`/usr/share/gamebian/steam-firstboot-terminal.sh`** (plain Steam; finish login/updates, then press Enter in that terminal to create the marker). If Steam was already used before (**`~/.local/share/Steam/config/loginusers.vdf`** exists), the terminal is skipped and the marker is created automatically. **Phase 2 —** once the marker exists, **each** login autostarts **nested gamescope** + **Steam gamepad UI** via **`/usr/local/bin/gamebian-steam-bigpicture`** (**`gamescope -e -f -- steam -gamepadui`**; **`GAMEBIAN_GAMESCOPE_WINDOWED=1`** drops **`-f`**). Quitting Steam exits gamescope and returns to Openbox; reopen from **Steam Big Picture** in the menu or **Super+Shift+S**. **`STEAM_BIGPICTURE_UI=classic`** uses **`-bigpicture`**. Disable Steam’s own **run at login** so you do not get two clients.
