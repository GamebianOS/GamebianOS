...
- Thin base: **`sudo`**, **`systemd-sysv`**, kernel metapackage, **`network-manager`**, **`nm-tray`**, **`qt5-gtk-platformtheme`**, **`qt6-gtk-platformtheme`**, **`hicolor-icon-theme`**, **`nm-connection-editor`**, **`openssh-client`**, **`openssh-server`**, **`iproute2`**; **`netsurf-gtk`** for **`x-www-browser`** (chroot hook sets alternatives). **`nm-tray`** starts **from Openbox autostart immediately after **`lxpanel`**** (embedded systray must exist first; avoids Qt **`No Icon set`**). Uses **`QT_QPA_PLATFORMTHEME=gtk3`** so Qt picks up GTK icon settings from **`lxappearance`**. **`/etc/systemd/system/multi-user.target.wants/ssh.service`** symlink ensures **`sshd`** is enabled after install (`Connection refused` on port 22 almost always means the **`ssh`** / **`ssh.socket`** unit was not listening).
