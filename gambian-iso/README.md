...
- Installer: **`calamares`** on the ISO only; **`calamares` / live-\* stacks are purged** on installed disk (**`packages`** job + APT autoremove). **LightDM** on the target first matches **`90-gamebian-openbox-first-session.conf`** (installed by **`shellprocess@gamebian-sshprep`**) so the initial session is **Openbox** until **`steam-firstboot-terminal.sh`** enables **`99-gamebian-steam-session`**. **`Build/share/calamares-gamebian/etc/calamares/modules/welcome.conf`** requires **internet** (HTTP checks on debian.org + fallbacks): you cannot proceed until the live session can reach the network mirror, avoiding silent APT **404**/offline failures during **`gamebian-web-install`**.
...
- **Steam (installed disk):** **Calamares `shellprocess@gamebian-sshprep`** writes **`/etc/lightdm/lightdm.conf.d/90-gamebian-openbox-first-session.conf`** (**autologin Openbox**) so fresh installs reach the desktop and **xfce4-terminal** can launch **`steam-firstboot-terminal.sh`** Steam install/sign-in wizard. Completing that script runs **`/usr/sbin/gamebian-enable-steam-lightdm-session`** ( **`sudo NOPASSWD`** for **`sudo`**) which removes **`90-…`** and writes **`99-gamebian-steam-session`** ( **`gamebian-steam-gamescope`** ). **`notify-send`**, **`xmessage`**, and **`zenity`** remind you **to reboot**. After reboot LightDM lands on fullscreen **gamescope** + **Steam**. From the greeter you can pick **Openbox** any time or relaunch **`/usr/local/bin/gamebian-steam-bigpicture`** under Openbox (**`Super+Shift+S`** / menu). **Hybrid GPU / Vulkan** tuning: **`steam-gamescope.env`** (see **`steam-gamescope.env.example`**). **`STEAM_BIGPICTURE_UI=classic`** still maps to **`-bigpicture`**. **`GAMEBIAN_GAMESCOPE_*`** flags are documented atop **`gamebian-steam-bigpicture`**.
